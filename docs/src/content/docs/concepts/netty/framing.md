...
The [event loop lab](../event-loop/) assumed each `channelRead` delivers exactly one application message. This lab proves that assumption false, then fixes it. Its only question is:
