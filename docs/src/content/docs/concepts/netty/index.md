...
Once a lab's code is proven, [Containerizing a Netty Service](../../operations/netty/deployment/) takes the event loop lab from `java -jar` on the host to a health-checked, non-root container image, with an executable smoke test proving the running container, not just the jar.
