...
This page proves one service can be built, run, and health-checked as a container. It does not cover multi-service orchestration, which the [Flink observability stack](../flink/observability-and-incidents/) already demonstrates for a different reason — Prometheus and Grafana coordinating with a running job, not one service in isolation.
