...
Those require external checkpoint storage, real connectors, process-level failure injection, and sink reconciliation. The operations handbook's [Recovery, Upgrade, and Rescaling](../../../operations/flink/recovery-upgrade-rescaling.md) page describes that broader drill.

For a planned stateful change rather than an unplanned failure, continue with the [Savepoint Upgrade and Rescaling Lab](savepoint-upgrade.md).
