...
Same pattern as the [state TTL and backend lab's](ttl-and-backends.md) backend selection: no fluent environment setter, just `Configuration` keys passed to `StreamExecutionEnvironment.getExecutionEnvironment(configuration)`.
...
This lab proves the retry _policy_ in isolation from state. The [checkpoint recovery lab](checkpoint-recovery.md) proves the complementary half: that state actually survives a restart once one happens. A production job depends on both being correct together.
